#!/bin/sh

#kubectl create secret generic vmi-lustre-oss-secret --from-file=userdata=lustre-oss-startup.sh
#export NEW_USER="foo"
#export SSH_PUB_KEY="ssh-rsa  moc-lustre"

export SSH_PUB_KEY="ssh-rsa  master@lustreproj"

#sudo adduser -U -m $NEW_USER
export DEFAULT_USER="centos"
echo "$DEFAULT_USER:centos" | chpasswd
sudo mkdir /home/$DEFAULT_USER/.ssh
sudo echo "$SSH_PUB_KEY" > /home/$DEFAULT_USER/.ssh/authorized_keys
sudo chown -R ${DEFAULT_USER}: /home/$DEFAULT_USER/.ssh

# enable lnet
if [ ! -c /dev/lnet ] ; then
   sudo exec /sbin/modprobe -v lnet >/dev/null 2>&1
fi

# enable lustre
/sbin/lsmod | /bin/grep lustre 1>/dev/null 2>&1
if [ $? -ne 0 ] ; then
   sudo /sbin/modprobe -v lustre >/dev/null 2>&1
fi

# enable ZFS
/sbin/lsmod | /bin/grep zfs 1>/dev/null 2>&1
if [ $? -ne 0 ] ; then
   sudo /sbin/modprobe -v zfs >/dev/null 2>&1
fi

# create and mount OST
sudo /usr/sbin/mkfs.lustre --ost --fsname=lustrefs --mgsnode=lustre-mgs.default-lustre@tcp0 --index=1 /dev/vdb > /dev/null 2>&1
sudo /usr/sbin/mkfs.lustre --ost --fsname=lustrefs --mgsnode=lustre-mgs.default-lustre@tcp0 --index=2 /dev/vdc > /dev/null 2>&1
sudo /usr/bin/mkdir /ost1
sudo /usr/bin/mkdir /ost2
sudo /usr/sbin/mount.lustre /dev/vdb /ost1
sudo /usr/sbin/mount.lustre /dev/vdc /ost2
